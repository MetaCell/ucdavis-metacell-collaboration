...
if __name__ == "__main__":
    ...
    for _, ss_row in ssmap.iterrows():
        dp, scp, ss = ss_row["data"], ss_row["scope"], ss_row["session"]
        if PARAM_SKIP_EXSISTING and os.path.exists(
            os.path.join(OUT_PATH, "{}-{}.nc".format(scp, ss))
        ):
        ...
        try:
            with open(os.path.join(IN_PARAM_FOLDER, "{}.yaml".format(scp))) as yf:
                param_anm = yaml.full_load(yf)
            param.update(param_anm)
            print("using config {}.yaml".format(scp))
        except FileNotFoundError:
            pass
        try:
            with open(
                os.path.join(IN_PARAM_FOLDER, "{}-{}.yaml".format(scp, ss))
            ) as yf:
                param_ss = yaml.full_load(yf)
            param.update(param_ss)
            print("using config {}-{}.yaml".format(scp, ss))
        except FileNotFoundError:
            pass
        ...
        try:
            ...
            with redirect_stdout(io.StringIO()):
                result_ds, plots = minian_process(
                    dpath=os.path.join(IN_DPATH, dp, "Miniscope"),
            ...
            with open(os.path.join(OUT_ERR_FOLDER, "-".join([scp, ss])), "w") as txtf:
                traceback.print_exception(None, err, err.__traceback__, file=txtf)
            client.close()
            cluster.close()
            continue
        result_ds = (
            result_ds.assign_coords(scope=scp, session=ss)
            .expand_dims(["scope", "session"])
            .compute()
        )
        result_ds.to_netcdf(
            os.path.join(OUT_PATH, "{}-{}.nc".format(scp, ss)), format="NETCDF4"
        )
        for plt_name, plt in plots.items():
            plt_name = "-".join([scp, ss, plt_name])
            save(
                hv.render(plt),
